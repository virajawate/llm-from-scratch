...
def main():
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % 50 == 0:
            print(f"step {step:5d} | loss {loss.item():.4f} | {(time.time()-t0):.1f}sec")
            t0 = time.time()
        ...
        if step % args.eval_interval == 0:
            ...
            if losses['val'] < best_val:
                best_val = losses['val']
                ckpt_path = f"{args.output_dir}/model_best.pt"
        ...
        if args.sample_every > 0 and step % args.sample_every == 0:
            ...
            output = model.generate(seed, max_new_tokens=args.sample_tokens, temperatur=args.temperature, top_k=args.top_k)
            text = tok.decode(output[0].cpu())
            print("\n=============== SAMPLE ===============\n")

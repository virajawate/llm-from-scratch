...
def compute_reward(reward_model: RewardModel, tok: RLHFTokenizer, prompt: str, response: str, device) -> float:
    ...
    with torch.no_grad():
        print("Reward input:")
        print("shape:", x.shape)
        print("min token:", x.min().item())
        print("max token:", x.max().item())
        print("reward vocab_size:", reward_model.vocab_size)
        print("embedding rows:", reward_model.tok_emb.num_embeddings)
        assert x.min() >= 0
        assert x.max() < reward_model.tok_emb.num_embeddings
        r = reward_model(x)

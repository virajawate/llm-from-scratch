...
class MoE(nn.Module):
    ...
    """
    def __init__(self, dim:int, n_expert:int, k:int=1, multi:int=4, swiglu:bool=True, dropout:float=0.0):
        super().__init__()
        self.dim = dim
        self.n_expert = n_expert
        self.k = k
        self.gate = TopKGate(dim, n_expert, k=k)
        self.expert = nn.ModuleList([ExpertMLP(dim, mult=multi, swiglu=swiglu, dropout=dropout) for _ in range(n_expert)])

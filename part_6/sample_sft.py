...
def main():
    ...
    with torch.no_grad():
        output = model.generate(idx, max_new_tokens=args.tokens, temperature=args.temperature, top_k=3)
    
    out_ids = output[0].tolist()
    origin_len = idx.size(1)
    if hasattr(col, "tok") and hasattr(col.tok, "decode"):
        generated = col.tok.decode(out_ids)
        print(generated)
    else:
        generated = bytes(out_ids[origin_len:]).decode("utf-8", errors="ignore")
        print(generated)

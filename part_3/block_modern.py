...
class TransformerBlockModern(nn.Module):
    def __init__(self, n_embd:int, n_head:int, dropout:float=0.0,
        ...
        self.attn = CausalSelfAttentionModern(n_embd, n_head, dropout, rope, max_pose, sliding_window, attention_sink, n_kv_head)
        self.ln2 = Norm(n_embd)
        self.ffn = SwiGLU(n_embd, multi=4, dropout=dropout) if use_swiglu else nn.Sequential(

...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        start = time.time()
        output = model.generate(prompt, max_new_tokens=args.tokens, temperature=0.0, top_k=50, 
                                top_p=None, sliding_window=args.sliding_window, attention_sink=args.sink)
        print(f"Generated {args.tokens} tokens in {time.time() - start:.2f} sec.")

        start = time.time()
        output_nocache = model.generate_nocache(prompt, max_new_tokens=args.tokens, temperature=0.0, 
                                                top_k=50, top_p=None, sliding_window=args.sliding_window, attention_sink=args.sink)
        print(f"(nocache) Generated {args.tokens} tokens in {time.time() - start:.2f} second.")
    print(tok.decode(output[0].cpu()))
    print(tok.decode(output_nocache[0].cpu()))
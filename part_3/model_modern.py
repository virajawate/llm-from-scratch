...
class GPTModern(nn.Module):
    def __init__(self, vocab_size:int=256, block_size:int=256,
                 ...
                 max_pose:int=4096, sliding_window:int | None = None, attention_sink:int=0,
        ...
        self.blocks = nn.ModuleList([
            TransformerBlockModern(n_embd, n_head, dropout, use_rmsnorm, use_swiglu, rope, max_pose, sliding_window, attention_sink, n_kv_head)
    ...
    @torch.no_grad()
    def generate_nocache(self, prompt:torch.Tensor, max_new_tokens=200, 
        ...
        for _ in range(max_new_tokens):
            ...
            print(f"top ids : {topi.tolist()}")
            print(f"top vs : {topv.tolist()}")
            next_id = torch.argmax(probs, dim=-1, keepdim=True) if temperature == 0.0 else torch.multinomial(probs, 1)
            idx = torch.cat([idx, next_id], dim=1)
        return idx
